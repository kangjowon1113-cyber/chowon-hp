...
export function DatingAlgorithmsPrototype({
  ...
  const bellButton = notificationCount > 0 ? (
  ...
  const pendingModal = pendingLikeProfile ? (
      ...
      {screen === "legacy" ? (
        ...
        <section className="relative flex h-full w-full items-center justify-center">
          <button
            type="button"
            onClick={() => setScreen("legacy")}
            className={`absolute top-4 z-20 rounded-full bg-white/80 px-4 py-2 text-xs font-bold text-[#4b4550] shadow-[0_4px_12px_rgba(0,0,0,0.12)] backdrop-blur-sm transition hover:bg-white ${notificationCount > 0 ? "right-24" : "right-4"}`}
          >
            Move to Study →
          </button>
          <div className="grid h-full w-full min-w-0 gap-5 grid-cols-[minmax(220px,302px)_minmax(0,1fr)] items-center">
            <div className="flex h-full min-h-0 min-w-0 items-center justify-center">
              <div className="relative aspect-[375/814] h-full max-h-full w-full max-w-[302px] rounded-[42px] border-[10px] border-[#c7c7cc] bg-[#c7c7cc] p-[8px] shadow-[0_18px_38px_rgba(0,0,0,0.22)]">
        ...
        <section className="flex h-full w-full flex-col">
          <div className="flex shrink-0 items-center gap-4 px-6 py-4">
            ...
            <button
              type="button"
              onClick={() => setScreen("legacy")}
              className={`shrink-0 rounded-full bg-white/80 px-4 py-2 text-xs font-bold text-[#4b4550] shadow-[0_4px_12px_rgba(0,0,0,0.12)] backdrop-blur-sm transition hover:bg-white ${notificationCount > 0 ? "mr-14" : ""}`}
            >
              Move to Study →
            </button>
          </div>
